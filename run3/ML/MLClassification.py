...
def data_prep(inputCfg, iBin, PtBin, OutPutDirPt, PromptDf, FDDf, BkgDf): #pylint: disable=too-many-statements, too-many-branches
    ...
    if dataset_opt == 'equal':
        ...
        PromptDfSelForEff = pd.concat([PromptDf.iloc[nCandToKeep:], TestSet.iloc[np.where(pd.Series(yTest).array == 1)]], sort=False)
        print(f"number of prompts for efficiency: {len(PromptDfSelForEff)}")
        if FDDf.empty:
            FDDfSelForEff = pd.DataFrame()
        else:
            FDDfSelForEff = pd.concat([FDDf.iloc[nCandToKeep:], TestSet.iloc[np.where(pd.Series(yTest).array == 2)]], sort=False)
            print(f"number of FD for efficiency: {len(FDDfSelForEff)}")
...
def train_test(inputCfg, PtBin, OutPutDirPt, TrainTestData, iBin): #pylint: disable=too-many-statements, too-many-branches
    ...
    if n_classes > 2:
        for Fig, Lab in zip(MLOutputFig, OutputLabels):
            Fig.savefig(f'{OutPutDirPt}/MLOutputDistr{Lab}_pT_{PtBin[0]}_{PtBin[1]}.png')
    else:
        MLOutputFig.savefig(f'{OutPutDirPt}/MLOutputDistr_pT_{PtBin[0]}_{PtBin[1]}.png')
    #_____________________________________________
    ...
    ROCCurveFig.savefig(f'{OutPutDirPt}/ROCCurveAll_pT_{PtBin[0]}_{PtBin[1]}.png')
    ...
    PrecisionRecallFig.savefig(f'{OutPutDirPt}/PrecisionRecallAll_pT_{PtBin[0]}_{PtBin[1]}.png')
    #_____________________________________________
    ...
    for iFig, Fig in enumerate(FeaturesImportanceFig):
        if iFig < n_plot:
            ...
            Fig.savefig(f'{OutPutDirPt}/FeatureImportance{label}_pT_{PtBin[0]}_{PtBin[1]}.png')
        else:
            Fig.savefig(f'{OutPutDirPt}/FeatureImportanceAll_pT_{PtBin[0]}_{PtBin[1]}.png')

    return ModelHandl

def plot_output_score_apply(score, lims, bins, label, color, kwds):
    plt.hist(score, color=color, alpha=0.5, range=lims, bins=bins,
...
def appl(inputCfg, PtBin, OutPutDirPt, ModelHandl, DataDfPtSel, PromptDfPtSelForEff, FDDfPtSelForEff):
    ...
    if not isinstance(df_column_to_save_list, list):
        ...
        sys.exit()
    if 'inv_mass' not in df_column_to_save_list and 'fM' not in df_column_to_save_list:
        print('\033[93mWARNING: inv_mass is not going to be saved in the output dataframe!\033[0m')
    if 'pt_candidate' not in df_column_to_save_list and 'fPt' not in df_column_to_save_list:
    ...
    if (not FDDfPtSelForEff.empty):
        print('Preparing output plot')
        fig, ax = plt.subplots(nrows = 1, ncols= 3, figsize = (21,8))
        for Pred, Lab in enumerate(OutputLabels):
            ax[Pred].hist(yPredData[:, Pred], bins = 100, range = (0,1), alpha = 0.5, histtype='stepfilled', label=f'Data ML_output_{Lab} pdf', density = True)
            ax[Pred].hist(yPredPromptEff[:, Pred], bins = 100, range = (0,1), alpha = 0.5, histtype='stepfilled', label=f'Prompt ML_output_{Lab} pdf', density = True)
            ax[Pred].hist(yPredFDEff[:, Pred], bins = 100, range = (0,1), alpha = 0.5, histtype='stepfilled', label=f'FD ML_output_{Lab} pdf', density = True) 
            ax[Pred].set_yscale('log')
            ax[Pred].set_title(f'ML output score {Lab} pdf')
            ax[Pred].set_xlabel('prob')
            ax[Pred].set_ylabel('entries (a.u.)')
            ax[Pred].legend()
        fig.savefig(f'{OutPutDirPt}/ModelAppliedOutputScore_pT_{PtBin[0]}_{PtBin[1]}.png')


def fillCandsHist (df):
    bincenters = (df['Pt_min'] + df['Pt_max'])/2
    bin_edges = np.append(np.array(df['Pt_min']), np.array(df['Pt_max'])[-1])
    fig, ax = plt.subplots(figsize=(21,10))
    ax.hist(bincenters,bins = bincenters, weights = df['nBkg'], histtype = 'step', label = 'Background')
    ax.hist(bincenters,bins = bincenters, weights = df['nPrompt'], histtype = 'step', label = 'Prompt')
    ax.hist(bincenters,bins = bincenters, weights = df['nFD'], histtype = 'step', label = 'Feed Down')
    ax.set_xlabel('Pt [GeV/c]', fontsize=18)
    ax.set_ylabel('N candidates', fontsize=18)
...
def main(): #pylint: disable=too-many-statements
    ...
    nPrompt, nFD, nBkg = [[] for _ in range(3)] 
    for iBin, PtBin in enumerate(PtBins):
        ...
        nPrompt.append(len(PromptHandler.get_slice(iBin)))
        nFD.append(len(FDDfPt))
        nBkg.append(len(BkgHandler.get_slice(iBin)))
        print(nPrompt, nFD,  nBkg)
    ...
    dfOut = pd.DataFrame( 
        {'Pt_min' : inputCfg['pt_ranges']['min'],
        'Pt_max' : inputCfg['pt_ranges']['max'],
        'nPrompt': nPrompt,
        'nFD': nFD,
        'nBkg': nBkg}
        )
    dfOut.to_csv(f"{inputCfg['output']['dir']}/candidates.csv")
    fig = fillCandsHist(dfOut)
    fig.savefig(f"{inputCfg['output']['dir']}/candidates.png")
main()
